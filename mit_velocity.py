...
class RoboticArmEnv(gym.Env):
    def __init__(self, render=False, random_target=False, *args, **kwargs):
        ...
        self.theta_dot = np.zeros(self.theta_dim)  # Initialize joint velocities
    ...
    def get_observation(self):
        return np.concatenate([self.theta, self.theta_dot,
    ...
    def reset(self):
        ...
        self.theta_dot = np.zeros(self.theta_dim)  # Reset joint velocities
        self.current_step = 0
        self.reward = 0
    ...
    def is_done(self, curr_distance):
        return curr_distance <= self.threshold or self.current_step == self.max_steps - 1

    def step(self, action):
        control_input = np.clip(action, -1, 1)
        joint_penalties = self.calculate_joint_penalties()

        # Directly use joint velocities from the action input
        self.theta_dot = control_input

        # Update joint angles
        self.theta += self.theta_dot
        self.theta = np.clip(self.theta, self.joint_min_angles, self.joint_max_angles)
